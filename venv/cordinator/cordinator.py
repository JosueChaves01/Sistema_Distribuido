...
async def upload_image(file: UploadFile = File(...)):
    ...
    target = min(workers.items(), key=lambda x: x[1]["ram"])[1]

    try:
        res = requests.post(f"http://{target['ip']}:8001/execute", json=task, timeout=10)
        return {"status": "sent", "to": target["ip"], "response": res.json()}
